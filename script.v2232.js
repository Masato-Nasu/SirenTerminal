'use strict';
// v22.3.2 — Minimal modal + robust Enter handling (IME/日本語入力対応) + JSONP + prefetch
(function(){
  ...
  // ====== Profile ======
  const PROFILE_KEY = 'siren_nl_profile_v2232';
  ...
  // ====== Minimal modal (robust Enter handling) ======
  ...
  // IME中は Enter を拾わない。compositionend 後の Enter/Keyup もケア。
  let composing = false;
  if (prefInput){
    ...
    // keydown/keyup 両方で拾う（ブラウザ差異対策）
    prefInput.addEventListener('keydown', e=>{
      if ((e.key === 'Enter' || e.keyCode === 13) && !composing){
        e.preventDefault(); trySave();
      }
    });
    prefInput.addEventListener('keyup', e=>{
      if ((e.key === 'Enter' || e.keyCode === 13) && !composing){
        e.preventDefault(); trySave();
      }
    });

    // SR/自動テスト向けの透明ボタンも発火させる
    if (ghost) ghost.addEventListener('click', trySave);

    // モーダル外クリックでは閉じない（誤タップ防止）。Esc は閉じるだけ保存しない。
    ...
    window.addEventListener('keydown', e=>{
      ...
      // 予備: Ctrl+Enter でも保存
      if ((e.ctrlKey || e.metaKey) && (e.key === 'Enter' || e.keyCode === 13) && !modal.classList.contains('hidden')){
        e.preventDefault(); const val = (prefInput.value||'').trim(); if (val || val===''){ const click = ghost; click && click.click(); }
      }
    });
  }
