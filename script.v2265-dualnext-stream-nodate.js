'use strict';
// v22.6.5 Dual NEXT / Stream Related + Noise Filter
// - 日付・年・「一覧」等のノイズタイトルをフィルタ
// - ランダムもフィルタを通す（最大5回まで再試行）
(() => {
  if (window.__siren_lock_v2265DNF) return; window.__siren_lock_v2265DNF = true;
  ...
  const keep = /script\.v2265-dualnext-stream-nodate\.js/i;
  ...
  // Noise filter
  function isNoisyTitle(t){
    if(!t) return true;
    // Namespaces or meta
    if (/^(Wikipedia|ウィキペディア|Help|ヘルプ|Portal|ポータル|Template|テンプレート|Category|カテゴリ|ファイル):/i.test(t)) return true;
    // Year or date patterns
    if (/^\d{3,4}$/.test(t)) return true;                            // 1998
    if (/^\d{3,4}年代$/.test(t)) return true;                         // 1990年代
    if (/^\d{1,2}月\d{1,2}日$/.test(t)) return true;                   // 5月12日
    if (/^\d{3,4}年(\d{1,2}月(\d{1,2}日)?)?$/.test(t)) return true;     // 2024年 / 2024年5月 / 2024年5月12日
    if (/^(令和|平成|昭和)\d+年$/.test(t)) return true;                 // 令和5年 等
    if (/紀元前\d+年/.test(t)) return true;
    // List-like
    if (/(一覧|年表|年譜|のリスト|リスト|作品一覧|人物一覧)$/.test(t)) return true;
    // Day-topic meta
    if (/(今日は何の日|記念日|出来事|誕生日|死亡|没)$/.test(t)) return true;
    return false;
  }
  ...
  async function randomTitleFiltered(maxTry=5){
  ...
  async function prefixTitles(q){
    ...
    return (Array.isArray(j) && Array.isArray(j[1])) ? j[1].filter(goodTitle) : [];
  }
  async function pageLinks(title, plcontinue){
    ...
    const links = (ps[0]?.links||[]).map(o=>o.title).filter(goodTitle);
  ...
  async function searchTitlesPaged(q, offset){
    ...
    const j=await jsonp(u);
    const arr=(j?.query?.search||[]).map(o=>o.title).filter(goodTitle);
    const nextOffset = (j?.continue?.sroffset != null) ? j.continue.sroffset : null;
    return {titles:arr, nextOffset};
  }

  // LRU
  const LRU_KEY='siren_seen_titles_v2265DNF';
  ...
  // Seed
  function sanitizeSeed(q){
    // 日付・年だけの seed は弱いので除外
    if (!goodTitle(q)) return '';
    return q;
  }
  ...
  async function nextRelatedOne(seed, curTitle){
    ...
    if (curTitle) {
      try{
        ...
        st.plcontinue = cont; saveState(seed, st);
  ...
  async function showNextRelated(){
    ...
    try{
      ...
      if(!s){
        const t = await randomTitleFiltered(5);
  ...
  function applyQuery(){
    ...
    (async()=>{
      ...
      try{
        ...
        if(!s){
          const t = await randomTitleFiltered(5); s = await getSummary(t);
        }
        render(s, token);
