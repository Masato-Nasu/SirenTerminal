'use strict';
// v22.4.2 — No modal/no hints, auto-run, anti-double, related-first
(() => {
  if (window.__siren_lock_v2242) return; window.__siren_lock_v2242 = true;

  // Kill other script.js/script.v*.js tags (anti double-load)
  const keep = /script\.v2242\.js/i;
  ...
  // No-repeat LRU
  const LRU_KEY = 'siren_seen_titles_v2242';
  ...
  // AUTO RUN on load (no modal, no hints)
  const boot = () => showNext();
