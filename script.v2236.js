'use strict';
// v22.3.6 — Related-first guarantee
(function(){
  ...
  // ===== Profile =====
  const PROFILE_KEY = 'siren_nl_profile_v2236';
  ...
  async function getLinks(title){
    ...
    const p = pages[0];
    return (p.links||[]).map(x=>x.title).filter(Boolean);
  }
  async function getSearchRelated(seed){
    ...
    const j = await jsonp(u, 8000);
    return (j.query && j.query.search) ? j.query.search.map(o=>o.title) : [];
  }
  ...
  async function refillRelatedFromTitle(seedTitle){
    const pool = new Set();
    try { for (const t of await getLinks(seedTitle)) pool.add(t); } catch(_){}
    try { for (const t of await getSearchRelated(seedTitle)) pool.add(t); } catch(_){}
  ...
  async function showOneInstant(){
    // 1) 関連キューを最優先。なければ、プロファイルから関連を補充
