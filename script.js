// v20.2: category pagination + daily reset + strong seed + bugfixes
(function(){
...
let current=null;const V='v20_2',SEEN_KEY='siren_seen_titles_'+V,DAY_KEY='siren_seen_day_'+V,CURSOR_KEY='siren_cat_cursor_'+V,SEEN_LIMIT=1e5;
...
async function sSeed(){
  const n=Math.floor(Date.now()/1e3),p=(performance.now()*1e3|0)&0xffffffff,r=crypto.getRandomValues(new Uint32Array(2)),u=navigator.userAgent;
...
// Wikipedia helpers
async function catBatch(cat,cmc=""){
  const u='https://ja.wikipedia.org/w/api.php?action=query&format=json&list=categorymembers&cmtitle='+encodeURIComponent('Category:'+cat)+'&cmtype=page&cmnamespace=0&cmlimit=100&origin=*'+(cmc?('&cmcontinue='+encodeURIComponent(cmc)):'');
  const d=await jget(u);
  const m=(d.query&&d.query.categorymembers)||[];
  const cont=(d.continue&&d.continue.cmcontinue)||"";
  return {titles:m.map(x=>x.title),cont};
}
async function titlesPaged(g,seed,need=80){
  let acc=[],cur=getCur(g);
  for(let i=0;i<6&&acc.length<need;i++){
    const {titles,cont}=await catBatch(g,cur);
    if(!titles.length&&!cont){cur="";setCur(g,cur);break;}
    acc.push(...titles);
    cur=cont||""; setCur(g,cur);
    acc=shuffle(acc,seed+BigInt(i));
  }
  acc=acc.filter(t=>!seenSet.has(t));
...
async function pickNew(){
  const g=genreSel.value; const seed=await sSeed(); let ts=[];
  if(g==='all'){ ts=(await rndTitles(60)).filter(t=>!seenSet.has(t)); shuffle(ts,seed); }
  else { ts=await titlesPaged(g,seed,80); }
  let tries=0;
  while(ts.length===0&&tries<4){
    tries++;
    if(g!=='all'){ setCur(g,""); ts=await titlesPaged(g,seed+BigInt(tries),80); }
    if(ts.length===0){ const mix=await rndTitles(60); shuffle(mix,seed+BigInt(256*tries)); ts=mix.filter(t=>!seenSet.has(t)); }
  }
  if(!ts.length) return null;
  return await sumByTitle(ts[0]);
}
async function showOne(){
  ...
  current=s; seenSet.add(s.title); saveSeen(); renderMain(s);
}
