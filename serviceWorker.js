// v19.18 network-first
const CACHE_NAME='siren-terminal-v19-18';
...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith(fetch(e.request,{cache:'no-store'}).then(r=>{const c=r.clone();caches.open(CACHE_NAME).then(ch=>ch.put(e.request,c));return r;}).catch(()=>caches.match(e.request)));
});
