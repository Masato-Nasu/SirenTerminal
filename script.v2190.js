...
'use strict';
// no top-level await guard inserted


// Runtime guard: remove legacy script.js if accidentally loaded together
(function killLegacyScript(){
  const scripts = Array.from(document.scripts);
  const legacy = scripts.find(s => /\/script\.js(\?.*)?$/i.test(s.src));
  if (legacy) {
    console.warn('Removing legacy script.js to avoid double-binding...');
    legacy.parentNode && legacy.parentNode.removeChild(legacy);
  }
})();


// Remove CLEAR/閉じる UI completely
(function removeCLEAR(){
  const matchers = [/^\s*CLEAR\s*$/i, /^\s*閉じる\s*$/];
  const nodes = Array.from(document.querySelectorAll('button, a, [role="button"], [data-action]'));
  for (const n of nodes){
    const t = (n.textContent||'').trim();
    const da = (n.getAttribute('data-action')||'');
    const id = n.id||'', cls = n.className||'';
    if (matchers.some(re => re.test(t)) || /clear/i.test(da) || /(^|[^a-z])clear([^a-z]|$)/i.test(id) || /(^|[^a-z])clear([^a-z]|$)/i.test(cls)){
      n.remove();
    }
  }
  try { if (window.clearBtn) window.clearBtn = null; } catch(e){}
})();


// Remove RELATED/関連 UI completely
(function removeRELATED(){
  const matchers = [/^\s*RELATED\s*$/i, /^\s*関連\s*$/];
  const nodes = Array.from(document.querySelectorAll('button, a, [role="button"], [data-action]'));
  for (const n of nodes){
    const t = (n.textContent||'').trim();
    const da = (n.getAttribute('data-action')||'');
    if (matchers.some(re => re.test(t)) || /related/i.test(da) || /(btn-)?related/i.test(n.id||'') || /(btn-)?related/i.test(n.className||'')){
      n.remove();
    }
  }
  // Also neuter global var if present
  try { if (window.relatedBtn) window.relatedBtn = null; } catch(e){}
})();
...
// SW
if (location.protocol.startsWith('http') && 'serviceWorker' in navigator) { try { navigator.serviceWorker.register('./serviceWorker.js').then(reg=>{ try{ if (reg && reg.update) reg.update(); }catch(e){} }); } catch(e){} }catch(e){} }).catch(()=>{}); }).catch(()=>{}); }
